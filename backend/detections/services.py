...
def summarize_payload_changes(previous_payload: dict | None, next_payload: dict | None) -> list[dict]:
    ...
    tracked = [
        ...
        ("risk_entity_type", "Risk Entity Type"),
        ("risk_entity_field", "Risk Entity Field"),
        ("risk_entities", "Risk Entities"),
        ("name", "Rule Name"),
        ("enabled", "Enabled"),
        ("type", "Rule Type"),
...
def build_local_rule_payload(
    *,
    ...
    if current_rule:
        ...
        payload.setdefault("entity_risk_enabled", False)
        payload.setdefault("risk_entities", [])
    else:
        payload = {
    ...
    if entity_risk_enabled is not None:
        payload["entity_risk_enabled"] = bool(entity_risk_enabled)
    if alert_mode is not None:
        payload["alert_mode"] = str(alert_mode)
    if risk_entity_type is not None:
        payload["risk_entity_type"] = str(risk_entity_type)
    if risk_entity_field is not None:
        payload["risk_entity_field"] = str(risk_entity_field)
    if risk_entities is not None:
        payload["risk_entities"] = risk_entities
    ...
    *,
    ...
    with transaction.atomic():
        ...
        payload = build_local_rule_payload(
            ...
            yaml_text=yaml_text,
            current_rule=rule,
            entity_risk_enabled=entity_risk_enabled,
            alert_mode=alert_mode,
            risk_entity_type=risk_entity_type,
            risk_entity_field=risk_entity_field,
...
def import_rule_files(*, files, actor: str) -> dict:
    ...
    def import_bundle_entry(entry: dict, source_name: str) -> None:
        ...
        save_local_rule(
            ...
            yaml_text=yaml_text,
            actor=actor,
            entity_risk_enabled=entry.get("entity_risk_enabled") if isinstance(entry.get("entity_risk_enabled"), bool) else None,
            alert_mode=entry.get("alert_mode") if entry.get("alert_mode") in {"risk", "notable"} else None,
            risk_entity_type=entry.get("risk_entity_type"),
            risk_entity_field=entry.get("risk_entity_field"),
            risk_entities=entry.get("risk_entities") if isinstance(entry.get("risk_entities"), list) else None,
