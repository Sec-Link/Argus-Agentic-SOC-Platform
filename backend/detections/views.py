...
class DetectionRuleDetailView(APIView):
    ...
    def get(self, request, rule_id: str):
        ...
        if payload.get("esql_source") == "manual":
            compiled = {
                **compiled,
                "language": "esql",
                "esql": str(payload.get("esql") or ""),
                "esql_source": "manual",
            }
        else:
            compiled = {
    ...
    def post(self, request, rule_id: str):
        ...
        rule = save_local_rule(
            ...
            actor=actor,
            entity_risk_enabled=serializer.validated_data.get("entity_risk_enabled"),
            alert_mode=serializer.validated_data.get("alert_mode"),
            risk_entity_type=serializer.validated_data.get("risk_entity_type"),
            risk_entity_field=serializer.validated_data.get("risk_entity_field"),
            risk_entities=serializer.validated_data.get("risk_entities"),
            ...
            schedule_interval=serializer.validated_data.get("schedule_interval"),
            schedule_from=serializer.validated_data.get("schedule_from"),
            esql=serializer.validated_data.get("esql"),
            esql_source=serializer.validated_data.get("esql_source"),
        )
        return Response({"saved": True, "id": rule.rule_uuid, "version": rule.version, "payload": rule.payload or {}})
